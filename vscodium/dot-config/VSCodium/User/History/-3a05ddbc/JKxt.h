...
/**
* @brief This functions set up internal PWM clock 0 to generate an interrupt at
* a 40 kHz firing rate
*/
void setup_pwm_interrupt();

/**
* @brief function to be called on each interrupt cycle, handles the ON/OFF of
* the PWM pins
...
*/
void inline turn_off(uint8_t *port, uint8_t pin){
    *port &= (0xFF & (~(_BV(pin)))); 
}
